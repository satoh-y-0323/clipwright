...
| `clipwright-color` | `clipwright_detect_color` | FFmpeg `signalstats` で平均輝度（v0.3.0 からはクロマキャストも）を測定し、フルグレーディングディレクティブを OTIO タイムラインのメタデータに書き込む。オートホワイトバランスは `UAVG`/`VAVG` の偏差からクロマキャストを補正し（`colorchannelmixer` のチャネル別ゲインとして格納、中立値 1.0、範囲 [0.0, 4.0]）、呼び出し元から指定した `saturation` / `contrast` / `gamma` は `eq` ブロックに書き込まれる。オプションの `.cube` ファイルパスで 3D-LUT グレーディングも適用可能。すべてのグレードステージは `clipwright-render` が WB（`colorchannelmixer`）→ eq → `lut3d` の順で一括レンダリングパスで適用する。新フィールドはすべて optional で、v0.2.x 以前のディレクティブとの後方互換性を維持する。 |
