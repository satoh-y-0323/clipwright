...
def _conform_write_copy(
    ...
    (ADR-NI-7, §13.3), then the shared ADR-EX-12/13 frame quantization, then
    (FCPXML only) the ADR-EX-13 available_range synthesis. *rep_rate* must be
    the same representative rate used as verify_rate in _write_adapter
    (invariant). Returns the warnings generated by the EDL and quantization
    steps, in generation order; the fcpxml synthesis step never warns.
    Mutates *tl_copy* in place.
    """
