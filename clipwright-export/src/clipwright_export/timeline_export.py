...
def _synthesize_missing_available_ranges(tl: otio.schema.Timeline) -> int:
    ...
    Only ExternalReference is handled. MissingReference is safe without this
    helper: its is_missing_reference is True, so the adapter's own fallback
    (item.duration() / source_range.start_time) applies and available_range
    is never dereferenced. GeneratorReference is an out-of-scope known
    limitation not addressed by this ADR: its is_missing_reference is False
    (so the adapter's missing-reference fallback does not apply) and its
    available_range is None by default, producing the same AttributeError
    on dereference; it also lacks a target_url attribute, which
    _clip_format_name dereferences unconditionally, causing a second,
    distinct AttributeError regardless of is_missing_reference. clipwright
    itself never generates this clip shape. Mutates *tl* (the write-time deep
    copy) in place; the source OTIO is never modified. Returns the number of
    references synthesized. Never raises.
    """
    synthesized = 0
    for clip in _iter_clips(tl):
        mr = clip.media_reference
...
def _conform_edl_tracks(tl_copy: otio.schema.Timeline, rep_rate: int) -> list[str]:
    """Remove EDL-unsupported audio tracks and emit the EDL-only warnings.

    CR-Q-002 (§13.3, ADR-NI-7): extracted out of _conform_write_copy so that
    function stays within the ~50-line guideline. Covers, in order: the
    audio-track-dropped warning (only if there are audio tracks), the actual
    removal of those Audio tracks from *tl_copy*, then the EDL rate warning
    (always emitted, EDL timecode carries no frame rate). Returns the
    warnings in generation order. Mutates *tl_copy* in place.
    """
    warnings_out: list[str] = []
    audio_tracks = [t for t in tl_copy.tracks if t.kind == otio.schema.TrackKind.Audio]
    if audio_tracks:
        warnings_out.append(
...
def _conform_write_copy(
    tl_copy: otio.schema.Timeline, fmt: str, rep_rate: int
) -> list[str]:
    """Apply the format-specific write-time conform steps to *tl_copy*.

    Covers, in order: EDL-only audio track removal + EDL rate warning
    (ADR-NI-7, §13.3, delegated to _conform_edl_tracks), then the shared
    ADR-EX-12/13 frame quantization, then (FCPXML only) the ADR-EX-13
    available_range synthesis. *rep_rate* must be the same representative
    rate used as verify_rate in _write_adapter (invariant). Returns the
    warnings generated by the EDL and quantization steps, in generation
    order; the fcpxml synthesis step never warns. Mutates *tl_copy* in place.
    """
    warnings_out: list[str] = []

    # --- Step 6c: EDL-specific warnings + audio removal (ADR-NI-7, §13.3) ---
    if fmt == "edl":
        warnings_out.extend(_conform_edl_tracks(tl_copy, rep_rate))
...
def _export_timeline_inner(
    ...
    # --- Step 6c/ADR-EX-12/13: format-specific write-time conform ---
    warnings_out.extend(_conform_write_copy(tl_copy, fmt, rep_rate))
