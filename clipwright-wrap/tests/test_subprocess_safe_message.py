"""test_subprocess_safe_message.py — Tests pinning wrap's subprocess-error message to core.

SR I-1 / CR-M-001.

These tests assert that:
  1. wrap.py does NOT define a module-level local _SUBPROCESS_SAFE_MESSAGE (the local
     copy was removed in favour of the shared core import).
...
Why the structural check (not hasattr) matters:
  wrap's local constant value "internal subprocess failed" was byte-identical to the core
  constant, so string comparison would pass even with a local copy.  The structural check
  confirms the migration happened; the envelope message assertions serve as contract checks.
...
def test_wrap_module_has_no_local_subprocess_safe_message() -> None:
    """wrap.py must NOT define a module-level _SUBPROCESS_SAFE_MESSAGE.

    The local copy was removed and the core import is used instead (SR I-1 migration).

    Why this assertion is necessary (prior lesson):
        The local constant value is byte-identical to the core constant, so
        message-equality checks would pass even with a local copy.  This test catches
        the structural change (removal of the local copy) that message-equality cannot see.
    """
