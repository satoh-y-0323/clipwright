...
| `clipwright-color` | `clipwright_detect_color` | Measure average luma (and, from v0.3.0, chroma cast) via FFmpeg `signalstats` and write a full grading directive to OTIO timeline metadata. Auto white-balance corrects chroma cast from `UAVG`/`VAVG` deviation (stored as a `colorbalance` directive). Caller-supplied `saturation`, `contrast`, `gamma` are written into the `eq` block. An optional `.cube` file path enables 3D-LUT grading. All grade stages are applied in a single render pass by `clipwright-render` in the order: WB (`colorbalance`) → eq → `lut3d`. All new fields are optional; existing v0.2.x directives are backward-compatible. |
