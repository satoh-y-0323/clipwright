"""test_noise_nle.py -- Resolve NLE conform wiring in detect_noise (Issue #2).

Context (architecture-report-20260715-191151.md §9 ADR-NI-10 rev.2 / FR-9 / NFR-4):
  detect_noise's new-creation path (timeline=None) calls
...
Mock policy (mirrors test_noise.py / test_available_range.py):
  - Patch clipwright_noise.noise.inspect_media to inject MediaInfo.
  - Patch clipwright_noise.noise.measure_noise to avoid ffmpeg.
...
_FAKE_MEASURE_RESULT: dict[str, Any] = {
...
class TestNewTimelineWithTimecode:
    def test_global_start_time_set_and_marker_present(self, tmp_path: Path) -> None:
        ...
        assert result.ok is True, f"detect_noise failed: {result.error}"
