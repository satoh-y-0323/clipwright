"""test_reframe_nle.py -- Resolve NLE conform wiring in reframe() (Issue #2).

Context (architecture-report-20260715-191151.md §9 ADR-NI-10 rev.2 / FR-9 / NFR-4):
  reframe()'s new-creation path (timeline=None) calls
  clipwright.nle_interop.conform_timeline_for_nle right after _add_full_clip
  builds the V1 (+A1 when has_audio) full-length keep clip(s), and merges its
  returned warnings into the tool's own warnings list. The existing-timeline
  (transform) path just loads the timeline verbatim and is never conformed
  (NFR-4).

Mock policy (mirrors test_reframe.py / test_available_range.py):
  - Patch clipwright_reframe.reframe.inspect_media to inject MediaInfo.
...
Verification points:
  (a) New-creation path, TC + 2ch audio: global_start_time is set from the
      ...
      marker, the pre-existing A1 mirror clip (present because has_audio is
      True) is *adopted* in place (Channels + Link Group ID stamped, no new
      track appended, no "audio mirroring skipped" warning), and both V1 and
      A1 clips' source_range are shifted by the same timecode-derived
      offset.
  (b) Existing-timeline (transform) path: the output timeline must not be
...
_OPTS = ReframeOptions(target_w=1080, target_h=1920, mode="pad")
...
class TestNewTimelineWithTimecode:
    def test_global_start_time_set_and_marker_present(self, tmp_path: Path) -> None:
        ...
        with patch(
            ...
            side_effect=lambda p: media_info,
        ):
            result = reframe(
                media=str(media), output=str(output), options=_OPTS, timeline=None
            )

        assert result["ok"] is True, f"reframe failed: {result.get('error')}"
...
# ===========================================================================
# (b) Existing-timeline (transform) path must never be conformed (NFR-4)
# ===========================================================================
...
class TestExistingTimelineNotConformed:
    def test_existing_timeline_is_not_conformed(self, tmp_path: Path) -> None:
        ...
        assert tl_out.metadata.get("Resolve_OTIO") is None, (
            "Existing timeline (transform path) must not gain Resolve_OTIO metadata."
        )
        assert tl_out.global_start_time is None, (
            "Existing timeline (transform path) must not gain global_start_time."
        )
