...
Coverage:
  ...
  ADR-K8: version '0.5.2' in words OTIO metadata (drift guard for REL-01)
"""
...
class TestWordTimestampsTrueIntegration:
    ...
    def test_otio_clip_words_metadata_version(self, tmp_path: Path) -> None:
        """metadata['clipwright']['version'] must be '0.5.2' in OTIO (drift guard REL-01).
        ...
        assert len(clips) == 1, "Expected exactly one clip on V1 track"
        cw = clips[0].metadata.get("clipwright", {})
        assert cw.get("version") == "0.5.2", (
            f"metadata['clipwright']['version'] must be '0.5.2' (drift guard REL-01), "
