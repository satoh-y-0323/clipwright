...
- **`available_range` is now populated on the generated clip** (GitHub Issue #1). The full-length
  source clip's `MediaRef.available_range` is now set to the same `0..media duration` range as
  `source_range`, using `clipwright`'s corrected video-stream-based duration (see `clipwright`
  v0.6.1, which no longer inflates `MediaInfo.duration` with audio drift). `words` OTIO metadata
  `version` is bumped to `"0.5.2"` to match (drift guard).
