"""test_loudness_nle.py -- Resolve NLE conform wiring in detect_loudness (Issue #2).

Context (architecture-report-20260715-191151.md §9 ADR-NI-10 rev.2 / FR-9 / NFR-4):
  detect_loudness's new-creation path (timeline=None) calls
  ...
  must never be conformed (NFR-4): an input timeline is loaded and partially
  updated, never mutated by conform.

Mock policy (mirrors test_loudness.py / test_available_range.py):
  - Patch clipwright_loudness.loudness.inspect_media to inject MediaInfo.
  - Patch clipwright_loudness.loudness.measure_loudness to avoid ffmpeg.
...
Verification points:
  (a) New-creation path, TC + 2ch audio: global_start_time is set from the
      ...
      marker, the pre-existing A1 mirror clip (_add_full_clip's own A1 clip)
      is *adopted* in place (Channels + Link Group ID stamped, no new track
      appended, no "audio mirroring skipped" warning), and both V1 and A1
      clips' source_range are shifted by the same timecode-derived offset.
  (b) Existing-timeline (accumulate) path: the output timeline must not be
      ...
      unchanged (still exactly V1+A1), and the pre-existing V1 clip's
      source_range/metadata are byte-for-byte unchanged (NFR-4).
  (c) New-creation path, no timecode: no shift is applied (source_range
      ...
      stamped -- that part of conform runs unconditionally regardless of
...
_MEASURED_NONE = {"measured": None, "warnings": []}
...
class TestNewTimelineWithTimecode:
    def test_global_start_time_set_and_marker_present(self, tmp_path: Path) -> None:
        ...
        assert result.ok is True, f"detect_loudness failed: {result.error}"
    ...
    def test_a1_adopted_no_mirroring_skipped_warning(self, tmp_path: Path) -> None:
        ...
        assert not any("skipped" in w for w in result.warnings), (
            f"A1 must be adopted (mirror-match), not skipped: {result.warnings}"
        )
...
class TestExistingTimelineNotConformed:
    def test_existing_timeline_is_not_conformed(self, tmp_path: Path) -> None:
        ...
        output = tmp_path / "out.otio"
        # start_timecode is set here to prove that, even when a shiftable TC
        # is available, the existing-timeline path still skips conform.
        media_info = _make_media_info(str(media), start_timecode="01:00:00:00")
