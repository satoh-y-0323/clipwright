"""test_plan.py — Red-phase tests for derive_keep_ranges (pure logic, no mocks).
...
def test_b1_keep_full_duration() -> None:
    ...
    ranges, warnings = derive_keep_ranges(D, _keep((0.0, D)))
    assert ranges == [(0.0, D)]
    assert warnings == []
...
def test_b2_keep_clamp_beyond_duration() -> None:
    ...
    ranges, warnings = derive_keep_ranges(D, _keep((0.0, D + 5.0)))
    assert ranges == [(0.0, D)]
...
def test_b5_keep_two_ranges_order_preserved() -> None:
    ...
    ranges, warnings = derive_keep_ranges(D, _keep((10.0, 30.0), (50.0, 70.0)))
    assert len(ranges) == 2
...
def test_b5_keep_reversed_order_preserved() -> None:
    ...
    ranges, warnings = derive_keep_ranges(D, _keep((50.0, 70.0), (10.0, 30.0)))
...
def test_b6_keep_overlapping_not_merged() -> None:
    ...
    ranges, warnings = derive_keep_ranges(D, _keep((0.0, 10.0), (5.0, 15.0)))
    assert len(ranges) == 2
...
def test_b7_drop_middle_produces_two_ranges() -> None:
    ...
    ranges, warnings = derive_keep_ranges(D, _drop((30.0, 60.0)))
...
def test_b9_drop_from_start() -> None:
    ...
    ranges, warnings = derive_keep_ranges(D, _drop((0.0, 40.0)))
...
def test_b10_drop_to_end() -> None:
    ...
    ranges, warnings = derive_keep_ranges(D, _drop((60.0, D)))
...
def test_b11_drop_overlapping_merged_then_complement() -> None:
    ...
    ranges, warnings = derive_keep_ranges(D, _drop((0.0, 10.0), (5.0, 15.0)))
...
def test_b12_drop_adjacent_merged() -> None:
    ...
    ranges, warnings = derive_keep_ranges(D, _drop((0.0, 10.0), (10.0, 20.0)))
...
def test_b13_both_empty_returns_full_passthrough() -> None:
    ...
    options = TrimOptions()
    ranges, warnings = derive_keep_ranges(D, options)
    assert ranges == [(0.0, D)]
    assert warnings == []
...
def test_b15_keep_padding_overlap_not_merged() -> None:
    ...
    ranges, warnings = derive_keep_ranges(D, _keep_pad((10.0, 30.0), (32.0, 50.0), padding=5.0))
    assert len(ranges) == 2
    # Both padded and clamped
    assert abs(ranges[0][0] - 5.0) < _EPS
    assert abs(ranges[0][1] - 35.0) < _EPS
    assert abs(ranges[1][0] - 27.0) < _EPS
    assert abs(ranges[1][1] - 55.0) < _EPS
...
class TestKeepModeAdditional:
    ...
    def test_keep_clamp_negative_start_with_padding(self) -> None:
        ...
        ranges, warnings = derive_keep_ranges(D, _keep_pad((5.0, 20.0), padding=10.0))
    ...
    def test_keep_multiple_ranges_warnings_per_clamped(self) -> None:
        ...
        ranges, warnings = derive_keep_ranges(
...
class TestDropModeAdditional:
    ...
    def test_drop_padding_inward_shrinks_drop(self) -> None:
        ...
        """
        ranges, warnings = derive_keep_ranges(D, _drop((20.0, 80.0), padding=5.0))
        assert len(ranges) == 2
        assert abs(ranges[0][0] - 0.0) < _EPS
        assert abs(ranges[0][1] - 25.0) < _EPS
        assert abs(ranges[1][0] - 75.0) < _EPS
        assert abs(ranges[1][1] - D) < _EPS
    ...
    def test_drop_degenerate_after_padding_discarded(self) -> None:
        ...
        """
        ranges, warnings = derive_keep_ranges(D, _drop((40.0, 50.0), padding=6.0))
        assert ranges == [(0.0, D)]
    ...
    def test_drop_unordered_input_still_correct(self) -> None:
        ...
        ranges, warnings = derive_keep_ranges(D, _drop((60.0, 80.0), (10.0, 30.0)))
