...
class TestSummarizeTimelineWindow:
    ...
    def _make_timeline_with_out_of_window_none_source_range_clip(
        self,
    ) -> tuple[object, int, int]:
        """Create a timeline whose sole ``source_range=None`` clip sits at a
        track position never covered by the (offset, limit) windows T1-6
        exercises (CR-NEW fix: the shared
        ``_make_multi_track_timeline_with_markers`` fixture gives every clip
        a ``source_range``, so it can never produce a warning and made T1-6
        vacuous).

        Returns: (timeline, clip_count, marker_count)

        Global clip order (index is per-track, scan order is V1 then A1):
          - V1[0] = "V1_clip_0" (has source_range)      -> global index 0
          - V1[1] = "V1_clip_1_no_source_range" (None)   -> global index 1
          - V1[2] = "V1_clip_2" (has source_range)       -> global index 2
          - A1[0] = "A1_clip_0" (has source_range)       -> global index 3

        T1-6 exercises windows (0, 1) -> global index 0, (2, 1) -> global
        index 2, and (3, 1) -> global index 3. None of them include global
        index 1 (the ``source_range=None`` clip), so its warning is only
        visible via the whole-timeline scan that produces ``warnings``,
        never because a window happened to include it.
        """
        import opentimelineio as otio

        from clipwright.otio_utils import new_timeline

        tl = new_timeline("t1_6_windowed_out_none_source_range")
        v1_track = tl.tracks[0]
        a1_track = tl.tracks[1]
        ...
        # The clip windowed out of every T1-6 test case below.
        clip_v1_1_no_range = otio.schema.Clip(
            name="V1_clip_1_no_source_range",
            media_reference=otio.schema.ExternalReference(
                target_url="file:///v1_1.mov"
            ),
            source_range=None,
        )
        v1_track.append(clip_v1_1_no_range)

        clip_v1_2 = otio.schema.Clip(
            name="V1_clip_2",
            media_reference=otio.schema.ExternalReference(
                target_url="file:///v1_2.mov"
            ),
            source_range=otio.opentime.TimeRange(
                start_time=otio.opentime.RationalTime(0.0, 30.0),
                duration=otio.opentime.RationalTime(10.0, 30.0),
        ...
        v1_track.markers.append(marker)

        return tl, 4, 1  # clip_count=4, marker_count=1
    ...
    def test_t1_6_counts_and_warnings_window_independent(self) -> None:
        ...
        Regression coverage for the R-4 risk (§4.4): a warning generated by a
        ``source_range=None`` clip that a given window excludes must still
        appear in that window's ``warnings`` list, unchanged from the
        full-timeline scan. Uses a dedicated fixture (not the shared
        ``_make_multi_track_timeline_with_markers``, whose 5 clips all carry
        ``source_range`` and can never produce a warning) so this is a real
        check rather than a vacuous ``[] == []`` pass (CR-NEW).
        """
        from clipwright.otio_utils import summarize_timeline

        tl, clip_count, marker_count = (
            self._make_timeline_with_out_of_window_none_source_range_clip()
        )

        full_summary = summarize_timeline(tl)

        # Sanity precondition: the fixture must actually produce a warning,
        # otherwise the equality checks below would be vacuous again.
        assert len(full_summary["warnings"]) >= 1, (
            "Fixture must contain a source_range=None clip that warns;"
            " otherwise this test cannot detect R-4 (CR-NEW)"
        )

        # None of these windows includes global clip index 1 (V1's second
        # clip, the source_range=None one): (0,1) -> idx0, (2,1) -> idx2,
        # (3,1) -> idx3.
        for offset, limit in [(0, 1), (2, 1), (3, 1)]:
            summary = summarize_timeline(tl, clips_offset=offset, clips_limit=limit)

            # Confirm "outside the window" is actually true for this
            # construction, not just asserted in the docstring above.
            windowed_names = {c["name"] for c in summary["clips"]}
            assert "V1_clip_1_no_source_range" not in windowed_names, (
                f"Window (offset={offset}, limit={limit}) must exclude the"
                " source_range=None clip for this test to be meaningful"
            )
            ...
            # The core regression guard: the window-excluded clip's warning
            # must still be present, identical to the full-timeline scan.
    ...
    def test_t1_10_limit_zero_and_offset_past_end(
        ...
        Uses the patch-before-binding spy pattern (same fix as T1-2, §9.1 / §13.5):
        the original `_clip_to_dict` / `_marker_to_dict` references are captured
        *before* `monkeypatch.setattr` replaces the module attribute, and the spy
        delegates to those saved references. A spy that instead does
        `from clipwright.otio_utils import _clip_to_dict` *inside* its own body
        would, after monkeypatching, re-import itself and recurse infinitely if a
        regression ever caused it to be called -- turning a clear
        `AssertionError` into an opaque `RecursionError` (CR-NEW).
        """
        from clipwright.otio_utils import summarize_timeline
        ...
        # Capture the original references before monkeypatching, so the spies
        # forward to the real implementations instead of recursing into
        # themselves if ever actually called (§9.1 spy contract, §13.5 T1-2 fix).
        import clipwright.otio_utils as otio_utils

        original_clip_to_dict = otio_utils._clip_to_dict
        original_marker_to_dict = otio_utils._marker_to_dict
        ...
        assert call_counts["clip"] == 0, (
            f"_clip_to_dict must not be called for an empty (limit=0) window"
            f" (actual calls: {call_counts['clip']})"
        )
        assert call_counts["marker"] == 0, (
            f"_marker_to_dict must not be called for an empty (limit=0) window"
            f" (actual calls: {call_counts['marker']})"
        )
        ...
        assert summary["markers"] == []
        assert call_counts["clip"] == 0, (
            f"_clip_to_dict must not be called when offset is past the end"
            f" (actual calls: {call_counts['clip']})"
        )
        assert call_counts["marker"] == 0, (
            f"_marker_to_dict must not be called when offset is past the end"
            f" (actual calls: {call_counts['marker']})"
        )
