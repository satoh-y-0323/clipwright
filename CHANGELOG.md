...
- Only effects are written to the mirrors: their `source_range` and
  ...
  tracks — and timelines that were never conformed for an NLE behave
  exactly as before.
- New envelope field `data.mirrored_audio_clips_updated` (always present;
  `0` when the timeline has no mirrors). `summary` gains one extra
  sentence only when mirror clips were actually retimed.

### Changed (seven satellites)

- `clipwright-color` 0.4.1, `clipwright-reframe` 0.4.2,
  `clipwright-scene` 0.3.1, `clipwright-stabilize` 0.5.1,
  `clipwright-noise` 0.4.1, `clipwright-loudness` 0.4.1 and
  `clipwright-render` 0.19.2 now declare `clipwright>=0.7.2`, fulfilling
  the runtime prerequisite announced in the 0.38.6 notes. No code changes;
  `clipwright-render` additionally gains a regression test pinning the
  invisibility of mirrored audio tracks to the renderer.
...
- `clipwright_export_timeline` no longer converts unexpected exceptions
  during timeline loading to `OTIO_ERROR`; they now surface as `INTERNAL`,
  aligning with `clipwright_export_chapters` and all other tools.
- `clipwright_reframe` now returns `FILE_NOT_FOUND` (was `INVALID_INPUT`) for
  a missing timeline file, aligning with the rest of the suite; its hint
  still notes the `timeline` argument can be omitted.
- Malformed-OTIO error messages from export/reframe now come from
  `clipwright` core (basename-only wording) instead of local fixed strings;
  the `OTIO_ERROR` code is unchanged.

### Removed (`clipwright-export` v0.2.3, `clipwright-reframe` v0.4.1)

- The local malformed-JSON workarounds in `clipwright-export` (timeline
  export + chapters) and `clipwright-reframe`, as announced in the 0.38.4
  notes — `clipwright` core >= 0.7.1 performs the conversion canonically.

### Notes

- `clipwright-export` 0.2.3 and `clipwright-reframe` 0.4.1 now require
  `clipwright>=0.7.1`; core and all other satellite packages are unchanged.
