...
- **OTIO metadata goes in `metadata["clipwright"]` namespace**. Times use OTIO `opentime` (RationalTime / TimeRange), not float seconds.
  - **Exception: external NLE interop namespaces.** Metadata a third-party NLE reads by a fixed, vendor-defined key (not chosen by clipwright) is exempt from the `clipwright` namespace and is written under its own top-level key instead — e.g. `metadata["Resolve_OTIO"]` for the DaVinci Resolve audio-track-layout/link-group wire format `clipwright.nle_interop.conform_timeline_for_nle` writes (`Resolve OTIO Meta Version` / `Audio Type` / `Channels` / `Source Channel ID` / `Source Track ID` / `Link Group ID`). clipwright's own bookkeeping about that interop step (if any) still goes in `metadata["clipwright"]`; only the reader-mandated key/value shape is exempt. See `docs/clipwright-spec6.md`.
