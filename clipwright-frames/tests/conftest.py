"""Shared fixtures for clipwright-frames tests.

ffmpeg/ffprobe search order:
  1. PATH (shutil.which)
  2. Environment variables CLIPWRIGHT_FFMPEG / CLIPWRIGHT_FFPROBE

Integration tests are skipped only when the binary is not found.
Follows the same resolution policy as clipwright-scene/tests/conftest.py.
