...
(failing) against the pre-migration implementation because:
  - inline same-dir block (color.py L117-131) returns INVALID_INPUT for
...
class TestOutputInDifferentDirAllowed:
    ...
    RED against pre-migration code: inline same-dir block (color.py L117-131)
    ...
    def test_output_in_different_dir_succeeds(self, tmp_path: Path) -> None:
        ...
        RED: current implementation returns INVALID_INPUT (same-dir block L117-131).
        """
    ...
    def test_output_in_different_dir_with_timeline_colocated_succeeds(
        ...
        block (L117-131) causes the RED failure.
        """
        from clipwright_color.color import (  # type: ignore[import-not-found]
            detect_color,
        )
...
class TestMediaRefForOtioRule:
    ...
    def test_media_ref_absolute_when_media_outside_otio_dir(
        ...
        RED: fails because same-dir block (L117-131) prevents OTIO creation
