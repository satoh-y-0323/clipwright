"""test_color_nle.py -- Resolve NLE conform wiring in detect_color (Issue #2).

Context (architecture-report-20260715-191151.md §9 ADR-NI-10 rev.2 / FR-9 / NFR-4):
  detect_color's new-creation path (timeline=None) calls
...
Mock policy (mirrors test_color.py / test_available_range.py):
  - Patch clipwright_color.color.inspect_media to inject MediaInfo.
  - Patch clipwright_color.color.measure_brightness to avoid ffmpeg.
...
class TestNewTimelineWithTimecode:
    def test_global_start_time_set_and_marker_present(self, tmp_path: Path) -> None:
        ...
        with (
            patch("clipwright_color.color.inspect_media", return_value=media_info),
            patch(
        ...
        assert result["ok"] is True, f"detect_color failed: {result.get('error')}"
